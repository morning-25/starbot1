...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Star\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5562993332759:+55 62 993332759\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '.','!','#'
blocked = []
limitawal = '999999999'
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD MORNING STAR BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] MORNING STAR? Este é um recurso especial para o MORNING STAR ❌',
					ownerB: '[❗] MORNING STAR? Este é um recurso especial para o MORNING STAR ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de STAR. Registre-se para fazer amizade com o bot STAR por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar MORNING|17\n\n──「 STAR BOT 」──`,
			...
			const ownerNumber = ["5562993332759@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5562993332759@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5562993332759@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5562993332759@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5562993332759@s.whatsapp.net"] 
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIO MORNING BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @MORNING domina vida
          ...
          reply('Espere..')
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o morning ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT MORNING ✨* 」\n*│+ wa.me/5562993332759*╰──────*「 *MORNING* 」*────*\n\n*_SE QUER SER ADMIN DO BOT MORNING_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* MORNING🔥STAR/n*WPP:* wa.me/+5562993332759\n\n\nEspero que tenham gostado do bot mais lindo🔥😌'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
